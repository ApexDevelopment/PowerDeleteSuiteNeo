...
A fork of [Power Delete Suite by /u/j0be](https://github.com/j0be/PowerDeleteSuite) — check out the original README for full background on what PDS does and why it exists. This fork ships as a single self-contained Tampermonkey userscript with a handful of changes on top.

## What's different

- Single file install. No bookmarklet, no runtime GitHub fetches — everything is bundled into `powerdeletesuite.user.js`.
...
- When editing without custom replacement text, posts are overwritten with six random words followed by "This post has been redacted." The confirmation prompt tells you this before you proceed.
